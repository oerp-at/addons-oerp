...
class Profile(argparse.ArgumentParser):
    ...
    def get_envvar(self, name, default=None):
        ...
        if value is None:
            mappings = self.env_mapping.get(name)
            if mappings:
...
class Po_Export(ConfigCommand):
    ...
    def run_config(self):
        ...
        if self.params.pot or not self.params.lang:
            self.lang = None
...
class Test(ConfigCommand):
    ...
    def run_test(self,
        ...
        test_server_enabled = config.get('test_server')
        results = []
        for m in mods:
            ...
            suite = unittest.TestSuite(
                ...
                if (not position_tag or position_tag.check(t))
                    ...
                    and match_filter(t)
                    and (not isinstance(t, odoo.tests.common.HttpCaseCommon) or test_server_enabled)
            )
    ...
    def run_config_env(self, env):
        ...
        results = []
        if modules:
            server = self.get_server() if config.get('test_server') else None

            # start server if configured
            # no shutdown, because it's done with exit
            if server:
                # disable job queues before server is started
            ...
            time_taken = time.perf_counter() - start_time
            _logger.info("Tests executed in %.3fs", time_taken)
...
class Assemble(Command):
    ...
    def run(self, args):
        ...
        # sitepackages = [p for p in site.getsitepackages() if p.startswith(virtual_env)]
        # lib_path = sitepackages[0] if sitepackages else os.path.join(dir_workspace, "assembly")
        ...
        def setupAddons(only_links=False):
            ...
            linkFile(odoo_bin, os.path.join(bin_path, "odoo"))
...
class Backup(ConfigCommand, DBHelperMixin):
    ...
    def get_installed_modules(self, cr):
        cr.execute('''
            SELECT name
            FROM ir_module_module
            WHERE state IN ('installed', 'to upgrade', 'to remove');
        ''')
        return [result[0] for result in cr.fetchall()]
    ...
    def neutralize(self, env):
        # check if it is the right database
        if not self.db_name.endswith('_neutralized'):
            raise ConfigException(f'You cannot neutralize a database({self.db_name}) which not ends with _neutralized')

        _logger.info('Neutralize %s', self.db_name)
        cr = env.cr

        # double check if it is the right database
        cursor_db_name = cr.dbname
        if not cursor_db_name.endswith('_neutralized'):
            raise ConfigException(f'You cannot neutralize a database({cursor_db_name}) which not ends with _neutralized')

        installed_modules = self.get_installed_modules(cr)
        queries = self.get_neutralization_queries(installed_modules)
        for query in queries:
            cr.execute(query)

        # last but not least change database uuid
        cr.execute("UPDATE ir_config_parameter SET value = %s WHERE key = 'database.uuid'", (str(uuid.uuid4()),))
        cr.commit()
    ...
    def run_config(self):
        ...
        # neutralize database
        if self.params.neutralize:
            self.set_db_name(f'{self.db_name}_neutralized')
            self.restore_database(backup_file_path)
            self.setup_env(database=self.db_name)

    def run_config_env(self, env):
        if self.params.neutralize:
            # export neutralized data
            self.neutralize(env)
            neutralized_backup_dir = os.path.join(self.backup_dir, 'neutralized')
            if not os.path.exists(neutralized_backup_dir):
                _logger.info('Create backup directory %s', neutralized_backup_dir)
                os.makedirs(neutralized_backup_dir, exist_ok=True)

            neutralized_backup_file_path = self.create_backlog(os.path.join(neutralized_backup_dir, 'neutralized.dump'))
            self.backup_database(neutralized_backup_file_path, compatible=self.params.compatible)


class Restore(ConfigCommand, DBHelperMixin):
    ...
    def __init__(self):
        ...
        self.parser.add_argument(
            ...
            default=False,
            name="wait_for_data",
            help="Waits for data and try again if data wasn't able to find."
        )
        self.parser.add_argument(
            "--pg_admin_user",
            name="pg_admin_user",
    ...
    def restore_filestore(self, url):
        ...
        if url.netloc:
            ...
            if not os.path.exists(rsync_url):
                raise ConfigException(f'Filestore source path {rsync_url} does not exist.')
    ...
    def download_database(self, url):
        ...
        if url.netloc:
            ...
            self.sync_files(rsync_url, dest_path, info='Download Database')
            self.db_dump = dest_path

        else:
            if not os.path.exists(url.path):
                raise ConfigException(f"Path {url.path} no exist")
            if os.path.isdir(url.path):
                db_files = [f for f in os.listdir(url.path) if f.endswith('.dump') or f.endswith('.sql')]
                if not db_files:
                    raise ConfigException(f"No dump file found at {url.path}")
                self.db_dump = os.path.join(url.path, db_files[-1])
            else:
                self.db_dump = url.path

    def download_and_restore_filestore(self):
        # restore filestore
        if self.params.restore_fs:
            restore_url = urlparse(self.params.restore_fs)
            if not restore_url:
                raise ConfigException(f"Filesystem restore URL is invalid {self.params.restore_fs}")
        ...
        if self.params.restore_db:
            ...
            if not restore_url:
                raise ConfigException(f"Database restore URL is invalid {self.params.restore_db}")
            self.download_database(restore_url)

    def setup_db_env(self):
        super().setup_db_env()

        # create user if necessary
        if not self.params.pg_admin_user:
            return False
        ...
        def add_param(name, name2):
            value = config.get(name)
            if value:
                params.append(f"{name2}='{value}'")

        add_param("db_host", "host")
        add_param("db_port", "port")

        params.append(f"user='{self.params.pg_admin_user}'")
        if self.params.pg_admin_password:
            params.append(f"password='{self.params.pg_admin_password}'")
    ...
    def run_config(self):
        # init filestore directory
        db_name = config.get('db_name')
        if not db_name:
            raise ConfigException('No database specified')
        ...
        self.restored_file = os.path.join(self.filestore, 'restored')
        ...
        while True:
            try:
                self.download_and_restore_filestore()
                break
            except (ConfigException, subprocess.CalledProcessError) as e:
                if not self.params.wait_for_data:
                    raise e
                _logger.warning(str(e))
                _logger.warning('Waiting 5sec for data ...')
                time.sleep(5)
...
class Install(ConfigCommand):
    ...
    def run_config_env(self, env):
        self.install_module(env,  self.params.module)
...
class Serve(Command, DBHelperMixin):
    ...
    def run(self, cmdargs):
        ...
        def report_configuration_hook():
            ...
            if args.wait_for_restore:
                restored_file = os.path.join(config['data_dir'], 'filestore', config['db_name'], 'restored')
                while not os.path.exists(restored_file):
                    _logger.warning('Waiting 5s for %s...', restored_file)
                    time.sleep(5)
            ...
            if args.wait_for_database:
                ...
                while not self.is_database_ready():
                    _logger.warning('Waiting 5s for Database %s...', self.db_name)
                    time.sleep(5)
