...
class BmdReconcil(models.Model):
        ...
        try:
            ...
            if numbers:
                ...
                if found_invoice_count != len(numbers):
                    found_invoice_vals = invoice_obj.search_read(
                        check_domain, ["number"]
                    )
                    found_invoice_numbers = set(
                        [v["number"] for v in found_invoice_vals]
                    )
                    not_found_numbers = list(
                        set(numbers).difference(found_invoice_numbers)
                    )
                    ...
                    not_found_numbers = list(
                        set(not_found_numbers) - like_found_numbers
                    )
            ...
            for inv in invoices:
                ...
                try:
                    taskc.log(
                        "Ausgleich von Rechnung", ref="account.invoice,%s" % inv.id
                    )
    ...
    def _compute_reconciled(self):
        invoice_obj = self.env["account.invoice"]
        for obj in self:
            obj.reconciled = invoice_obj.search(
                [("bmd_reconcil_id", "=", self.id)], count=True
            )
