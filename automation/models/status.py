...
class TaskStatus(object):
    ...
    def _post_data(self, url: str, data: Dict[str, Any],
                   result_parser: Callable[[requests.Response], Any] = lambda res: None) -> Optional[Any]:
        if self.token:
            ...
            def write_data(cursor):
                def update_progress():
                    ...
                    if status and progress:
                        cursor.execute(
                            'UPDATE automation_task_stage SET progress = %s, status = %s WHERE id = %s',
                            (progress, status, stage_id)
                        )
                    elif progress:
                        cursor.execute(
                            'UPDATE automation_task_stage SET progress = %s WHERE id = %s', (progress, stage_id)
                        )
                    elif status:
                        cursor.execute('UPDATE automation_task_stage SET status = %s WHERE id = %s', (status, stage_id))
                    return stage_id
                ...
                if url == 'log':
                    ...
                    cursor.execute(
                        """
                        INSERT INTO automation_task_log(
                            create_date, write_date, create_uid, write_uid,
                            task_id, stage_id, pri, message, ref, code, data
                        )
                        VALUES (
                    ...
                    cursor.execute(
                        """
                        INSERT INTO automation_task_stage(
                            create_date, write_date, create_uid, write_uid,
                            task_id, name, parent_id, progress, status, total
                        )
                        VALUES (
                            NOW() AT TIME ZONE 'UTC', NOW() AT TIME ZONE 'UTC',
                            %s, %s, %s, %s, %s, %s, %s, %s
    ...
    def _post_stage(self, data):
        ...
        if self.local:
            ...
            return self._post_data(
                self.stage_path,
                data,
                result_parser=lambda res: int(res.text) if res.text.isdigit() else None
            )
