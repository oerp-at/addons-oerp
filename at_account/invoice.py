...
class account_invoice(osv.osv):
    ...
    def _create_payment(
        ...
        payment_rate = 1.0
        payment_rate_currency = (
            currency_id
            and self.pool["res.currency"].browse(cr, uid, currency_id, context=context)
            or currency
        )
        voucher_type = (
            invoice.type in ("out_invoice", "out_refund") and "receipt" or "payment"
        )
        ...
        if not period_id:
            period_ids = period_obj.find(
                cr, uid, dt=(date or util.currentDate()), context=context
            )
            if period_ids:
        ...
        move_lines_ids = move_line_obj.search(
            cr,
            uid,
            [
        ...
        if len(line_datas) == 1:
            ...
            # compensation
            if compensation_id:
                compensation = compensation_obj.browse(
                    cr, uid, compensation_id, context=context
                )
    ...
    def action_move_create(self, cr, uid, ids, context=None):
        ...
        for inv in self.browse(cr, uid, ids, context=context):
            perf_start = inv.perf_start or self._get_performance_start(
                cr, uid, inv, context
            )
            perf_end = max(
                inv.perf_end or self._get_performance_end(cr, uid, inv, context),
                perf_start,
            )
            self.write(
                cr,
                uid,
                [inv.id],
                {"perf_start": perf_start, "perf_end": perf_end,},
                context=context,
            )

        return res

    def _check_partner_account(
        self, cr, uid, inv, seq, partner_account_field, context=None
    ):
        account_tmpl = self.pool["ir.property"].get(
            cr, uid, partner_account_field, "res.partner"
        )
        ...
        if (
            ...
            and account_tmpl
            and (
                account_tmpl.id == inv.account_id.id
                or (
                    partner.parent_id
                    and partner.type == "invoice"
                    and partner.commercial_partner_id[partner_account_field].id
                    == inv.account_id.id
                )
            )
        ):
            ...
            partner_account_id = partner_obj.read(
                cr, uid, partner.id, [partner_account_field], context=copyContext
            )[partner_account_field][0]

            # check if partner account is
            # template account
            # if it is create a new wone
            if partner_account_id == account_tmpl.id or (
                partner.parent_id
                and partner.type == "invoice"
                and partner.commercial_partner_id[partner_account_field].id
                == partner_account_id
            ):
                code = self.pool["ir.sequence"].next_by_id(
                    cr, uid, seq.id, context=copyContext
                )
                new_account_id = self.pool["account.account"].copy(
                    cr,
                    uid,
                    account_tmpl.id,
                    {"code": code, "name": inv.partner_id.name, "reconcile": True},
                    context=copyContext,
                )

                # assign new account
                partner_data = {partner_account_field: new_account_id}
                partner_obj.write(
                    cr, uid, [partner.id], partner_data, context=copyContext
                )
            ...
            # check if default account was set, but there already exist and account
                ...
                account_tmpl.id == inv.account_id.id
                and partner_account_id != account_tmpl.id
            ):
                # change account for invoice
                self.write(
                    cr,
        ...
        for inv in self.browse(cr, uid, ids, context=context):
            ...
            if inv.type in ("out_invoice", "out_refund"):
                self._check_partner_account(
                    ...
                    uid,
                    inv,
                    company.account_customer_seq_id,
                ...
                self._check_partner_account(
                    ...
                    uid,
                    inv,
                    company.account_supplier_seq_id,
            ...
            if is_zero(invoice.amount_total):
                ...
                if is_zero(sum(line.debit - line.credit for line in lines)):
                    move_line_obj.reconcile(
                        cr, uid, [line.id for line in lines], context=context
                    )

        # reimport first attachment as receipt if it is an pdf
        invoice_ids = self.search(
            cr, uid, [("id", "in", ids), ("type", "in", ["in_invoice", "in_refund"])]
        )
        for oid in invoice_ids:
            attachment_ids = attachment_obj.search(
                cr, uid, [("res_model", "=", "account.invoice"), ("res_id", "=", oid)]
            )
            if len(attachment_ids) == 1:
                # get attachment
                attachment = attachment_obj.browse(
                    cr, uid, attachment_ids[0], context=context
                )
                if (
                    attachment.origin != "account.invoice.attachment.wizard"
                    and attachment.name
                    and attachment.name.lower().endswith(".pdf")
                ):
                    ...
                    if attachment_data:
                        ...
                        wizard_id = attachment_wizard_obj.create(
                            cr,
                            uid,
                            {"document": attachment_data},
                            context=wizard_context,
                        )
                        attachment_wizard_obj.action_import(
                            cr, uid, [wizard_id], context=wizard_context
                        )
                        # clean up
                        attachment.unlink()
    ...
    def _tax_amount(self, cr, uid, sid, context=None):
        ...
        for line_rec in invoice_rec.invoice_line:
            tax_calc = self.pool.get("account.tax").compute_all(
                cr,
                uid,
                line_rec.invoice_line_tax_id,
                line_rec.price_unit * (1 - (line_rec.discount or 0.0) / 100.0),
                line_rec.quantity,
                line_rec.product_id.id,
                line_rec.invoice_id.partner_id.id,
            )
    ...
    def name_search(
        ...
        if name:
            ids = self.search(
                cr, user, [("number", "=", name)] + args, limit=limit, context=context
            )
        if not ids:
            ids = self.search(
                cr,
    ...
    def _get_subject(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for invoice in self.browse(cr, uid, ids, context):
            if invoice.type == "out_invoice" and (
                invoice.state == "open" or invoice.state == "paid"
            ):
    ...
    def _qrcode(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for inv in self.browse(cr, uid, ids, context):
            ...
            qr = qrcode.QRCode(box_size=4.2, border=4)
            data = "\n".join(
                ["BCD", "001", "1", "SCT", bic, company, iban, amount, "", ref]
            )
    ...
    _columns = {
        "invoice_text": fields.function(
            _invoice_text, type="text", string="Invoice Text"
        ),
        "subject": fields.function(
            _get_subject, type="char", size=128, string="Subject"
        ),
...
class account_invoice_line(osv.osv):
        ...
        for obj in self.browse(cr, uid, ids, context):
            ...
            if invoice and product:
                ...
                if invoice.type in ("out_invoice", "out_refund"):
                    ...
                    taxes = product.supplier_taxes_id or (
    ...
    def _discount_amount(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids):
            if line.discount:
                ...
                taxes_with_discount = tax_obj.compute_all(
                    cr,
                    uid,
                    line.invoice_line_tax_id,
                    price_with_discount,
    ...
    _columns = {
        "price_subtotal_taxed": fields.function(
            _amount_line_taxed,
            string="Subtotal (Brutto)",
            digits_compute=dp.get_precision("Account"),
        ),
        "price_unit_untaxed": fields.function(
            _price_unit_untaxed,
            string="Price Unit (Netto)",
            digits_compute=dp.get_precision("Account"),
        ),
        "product_tax_ids": fields.function(
            ...
            help="Original Product Taxes without fiscal position",
        ),
        "discount_amount": fields.function(
            _discount_amount, type="float", string="Discount Amount", readonly=True
        ),
